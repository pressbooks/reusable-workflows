...
This reusable workflow runs `npm run test --if-present` (lint) and `npm run build --if-present` on PR branches.
When a `build` script is defined in `package.json`, built assets are automatically committed back to the PR branch
with a `[skip ci]` commit, so CI doesn't re-trigger. Developers should not commit dist files locally — they are
generated by this workflow. If no `build` script exists, the build step is silently skipped.
